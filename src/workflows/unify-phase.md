...
4. Run `close --plan ID` (legacy reviewed SUMMARY: add --input REVIEW.json). The helper renders a compact SUMMARY and updates STATE, existing manifest, ledger and receipts transactionally. Outcome, decisions, deviations and issues remain in SUMMARY; full AC/task qualifications, verification, files and skills stay in RESULT. Do not expand SUMMARY by copying those details back. Inspect its result; do not manually duplicate these writes. Failed work remains documented and routes to corrective planning. A repeated close is idempotent.
