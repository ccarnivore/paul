...
1. Use the selected PLAN, applicable project constraints and audit configuration already in this session; load missing context only. Audit is optional unless configured or requested. Check the existing AUDIT's plan hash and findings; reuse resolved unchanged findings rather than generating the same report again. Recheck changed assumptions and dependencies.
...
3. Classify findings as must-have, strongly-recommended or can-safely-defer, with evidence, impact and precise remediation. Apply must-have/strongly-recommended corrections within authorized planning scope using focused edits, without reprinting the entire PLAN. Material scope/architecture decisions need user input; retain deferred findings with rationale. Do not modify application code during a plan audit.
