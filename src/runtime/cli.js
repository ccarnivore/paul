...
const { sections } = require('./markdown');
const { recover } = require('./transaction');
const { update } = require('./update');
function main(args = process.argv.slice(2)) {
  const command = args.shift() || 'help', opts = {};
  const flags = ['project','plan','phase','budget','query','limit','signal','input','mode','file','heading'];
  while (args.length) {
  ...
  if (command === 'help' || command === '--help') return { usage: 'node <claude-config>/paul-framework/runtime/cli.js COMMAND [--project DIR]', commands: {
    ...
    history: '[--query TEXT] [--limit N] — search summary metadata and matching lines',
  ...
  switch (command) {
    ...
    case 'section': {
      ...
      const found = sections(text).filter(s => s.title.toLowerCase() === opts.heading.toLowerCase());
      if (found.length !== 1) throw new Error('Section missing or ambiguous; read the source file');
      return { file, sha256: digest(file), section: opts.heading, text: found[0].text };
    }
    case 'history': return project.history(root, opts.query, opts.limit ? Number(opts.limit) : 10);
